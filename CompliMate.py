...
class CompliMate:
    ...
    os.makedirs(self.meta_folder, exist_ok=True)
        ...
        self.processed_file = os.path.join(self.meta_folder, "processed.json")
        self.index_file = os.path.join(self.meta_folder, "section_index.faiss")
        self.paragraph_index_file = os.path.join(self.meta_folder, "paragraph_index.faiss")
        self.paragraphs_file = os.path.join(self.meta_folder, "paragraphs.pkl")

        # persistence for learned roles/shared headings
        self.roles_file = os.path.join(self.meta_folder, "roles_map.json")
        self.shared_file = os.path.join(self.meta_folder, "shared_items.json")

        # model + placeholders
        self.model = SentenceTransformer("multi-qa-MiniLM-L6-cos-v1")
        dim = self.model.get_sentence_embedding_dimension()
        # ------------------------
        # Phi-2 GGUF Configuration
        # ------------------------
        self.llm = Llama(
    ...
    def _load_documents(self):
        ...
        for filename in os.listdir(self.rag_folder_):
            lower = filename.lower()
            fullpath = os.path.join(self.rag_folder_, filename)
            ...
            key = os.path.relpath(source_path, self.rag_folder_)
    ...
    def generate_llama_answer(self, prompt, context):
        full_prompt = f"Context:\n{context}\n\nQuestion:\n{prompt}\n\nAnswer:"
        output = self.llm(full_prompt, max_tokens=200, temperature=0.7, stop=["\n\n"])
    ...
    def query(self, prompt, user_role, top_k=5):
        ...
        if fuzzy_results:
            for res in fuzzy_results:
                answer = self.generate_llama_answer(prompt, res['section_content'])
                output_parts.append(
                    f"**File:** {res['filename']}\n\n"
